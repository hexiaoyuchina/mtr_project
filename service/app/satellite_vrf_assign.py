...
- ``MTR_SATELLITE_VRF_PREFIX``：VRF 名前缀，默认 ``vbgp``，完整名为 ``{prefix}{IPv4 去点}``（如 ``10.133.152.233`` → ``vbgp10133152233``，见 ``vrf_naming.satellite_vrf_name``）。
...
def _vrf_names_for_ip(ip_s: str, prefix: str, vrf_override: Optional[str] = None) -> Tuple[str, str, str, str]:
    ...
    vrf = (vrf_override or "").strip() or satellite_vrf_name(ip_s, prefix)
    if vrf_override:
        dum = f"dum{last}"
        va = f"{vrf}a"
        vb = f"{vrf}b"
